...
def compare(real , fake, save_dir, col_prefix, CDF = True , Hist = True) :
  ...
  for i in range(max_col):   
      ...
      if Hist == True:
        histogram( real[: max_row ,i] ,fake[: max_row ,i], xlabel = col_prefix  + ' : Column ' + str(i + 1) , ylabel ='' , save_dir = save_dir)
  ...
  if option == 0:
    ...
    save_images(samples, [image_frame_dim, image_frame_dim], './samples/test_%s.png' % strftime("%Y%m%d%H%M%S", gmtime()))
      ...
      if config.shgan_input_type == 1: 
        ...
        with open('./samples/'+ config.dataset+ '/' + config.test_id+ '/'+ config.test_id + '_scaled_fake_tabular.pickle', 'rb') as handle:
...
def discriminator_sampling(input , lables , output_file, title , config, dcgan, sess) :
  ...
  for idx in xrange( input_size // chunk) :
    ...
    samples = sess.run(dcgan.sampler_disc, feed_dict={dcgan.inputs : sample_input, dcgan.y: y_one_hot , dcgan.y_normal : y })
